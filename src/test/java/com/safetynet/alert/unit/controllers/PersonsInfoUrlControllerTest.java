...
import com.safetynet.alert.controller.PersonsInfoUrlController;
...
@WebMvcTest(controllers=PersonsInfoUrlController.class)
public class PersonsInfoUrlControllerTest {

	@Autowired
	private MockMvc personsInfoUrlControllerMockMvc;
	
	@MockBean
	private FindPersonsByLastNameService mockFindPersonsByLastNameService;
	...
	@Test
	public void PersonsInfoUrlTest() throws Exception{
		...
		when(mockFindPersonsByLastNameService.findByLastName(anyString())).thenReturn(p);
		when(mockFindMedicalRecordsByFirstNameAndLastNameService.findByFirstNameAndLastName(anyString(), anyString())).thenReturn(m);
		
		personsInfoUrlControllerMockMvc
			.perform(get("/personInfo?firstName=allison&lastName=boyd"))
